 ...
 * @param rerankerProvider 尝试执行的精排 Provider
 * @param rerankerModel 精排模型或规则版本
 * @param rerankerFallbackProvider 降级后实际使用的 Provider
 * @param rerankerInputCount 送入精排器的候选数量
 * @param rerankerOutputCount 精排器输出的候选数量
 * @param rerankerFallback 是否发生精排降级
 * @param rerankerErrorCategory 精排失败的低基数错误类别
...
public record RagRetrievalTrace(
        ...
        int rerankerInputCount,
        int rerankerOutputCount,
