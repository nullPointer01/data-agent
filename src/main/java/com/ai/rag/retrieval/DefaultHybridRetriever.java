...
public class DefaultHybridRetriever implements HybridRetriever {
    ...
    @Override
    public List<RetrievalResult> retrieve(RagQueryAnalysis analysis, String tenantId, int candidateTopK,
            double minScore, List<String> sourceTypes) {
        return retrieveWithTrace(analysis, tenantId, candidateTopK, minScore, sourceTypes).results();
    }

    @Override
    public HybridRetrievalResult retrieveWithTrace(RagQueryAnalysis analysis, String tenantId, int candidateTopK,
            double minScore, List<String> sourceTypes) {
        if (analysis == null || !StringUtils.hasText(analysis.rewrittenQuery()) || !StringUtils.hasText(tenantId)) {
        ...
        List<EmbeddingMatch<TextSegment>> vectorMatches = vectorMemoryService.searchMatches(
                analysis.rewrittenQuery(), candidateTopK, minScore, tenantId, sourceTypes);
        long vectorTimeMs = System.currentTimeMillis() - vectorStartedAt;
        long fullTextStartedAt = System.currentTimeMillis();
        List<FullTextSearchResult> fullTextResults = fullTextRetriever.retrieve(analysis, tenantId,
