...
public class VectorIndexRegistry {
    ...
    /**
     * 在数量、元数据和主键全部校验后，按输入顺序更新批量索引记录。
     *
     * @param segments 已写入 Milvus 的片段
     * @param primaryKeys Milvus 返回的同序主键
     */
    public void recordAll(List<TextSegment> segments, List<String> primaryKeys) {
        if (segments == null || primaryKeys == null || segments.size() != primaryKeys.size()) {
            throw new IllegalArgumentException("向量索引注册数量不一致: segments=" + sizeOf(segments)
                    + ", primaryKeys=" + sizeOf(primaryKeys));
        }
        List<PendingIndexEntry> pendingEntries = new ArrayList<>(segments.size());
        for (int index = 0; index < segments.size(); index++) {
            TextSegment segment = Objects.requireNonNull(segments.get(index),
                    "向量索引片段不能为空: index=" + index);
            String primaryKey = requireText(primaryKeys.get(index), "primaryKey", index);
            String type = requireMetadata(segment, VectorMetadataKeys.TYPE, index);
            String id = requireMetadata(segment, VectorMetadataKeys.ID, index);
            String sourceId = requireMetadata(segment, VectorMetadataKeys.SOURCE_ID, index);
            String tenantId = segment.metadata().getString(VectorMetadataKeys.TENANT_ID);
            String userId = segment.metadata().getString(VectorMetadataKeys.USER_ID);
            pendingEntries.add(new PendingIndexEntry(type, id, segment.text(), primaryKey,
                    sourceId, tenantId, userId));
        }
        pendingEntries.forEach(entry -> record(entry.type(), entry.id(), entry.text(), entry.primaryKey(),
                entry.sourceId(), entry.tenantId(), entry.userId()));
    }
    ...
    private String requireMetadata(TextSegment segment, String key, int index) {
        if (segment.metadata() == null) {
            throw new IllegalStateException("向量索引片段缺少 metadata: index=" + index);
        }
        return requireText(segment.metadata().getString(key), key, index);
    }

    private String requireText(String value, String field, int index) {
        if (value == null || value.isBlank()) {
            throw new IllegalStateException("向量索引批次字段为空: field=" + field + ", index=" + index);
        }
        return value;
    }
    ...
    private record PendingIndexEntry(String type, String id, String text, String primaryKey,
            String sourceId, String tenantId, String userId) {
    }
