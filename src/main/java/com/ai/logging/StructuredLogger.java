...
public class StructuredLogger {
     ...
     *
     * @param record 安全执行记录
     * @param executorId Agent 执行者标识
     */
    public void logGovernedToolCall(AgentToolExecutionRecord record, String executorId) {
        Map<String, Object> payload = basePayload(TYPE_TOOL_CALL);
        payload.put("runId", record.runId());
        payload.put("toolCallId", record.toolCallId());
        payload.put("toolName", record.toolName());
        payload.put("executor", executorId);
        payload.put("risk", record.risk() == null ? "UNKNOWN" : record.risk().name());
        payload.put("status", record.status().name());
        payload.put("authorized", record.authorized());
        payload.put("attempts", record.attempts());
        payload.put("durationMs", record.durationMs());
        payload.put("argumentSummary", record.argumentSummary());
        payload.put("sanitized", record.sanitized());
        payload.put("truncated", record.truncated());
        write(payload);
    }
