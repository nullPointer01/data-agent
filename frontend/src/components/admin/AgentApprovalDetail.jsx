...
export function AgentApprovalDetail({ detail, loading, now, submitting, onClose, onDecision }) {
  ...
  const confirmation = useMemo(() => {
    ...
    return confirmAction === 'approve'
      ? `批准后，系统会恢复 Run ${detail.runId} 并执行这一个不可修改的沙箱动作。`
      : `拒绝后，Run ${detail.runId} 将结束且该动作不能再次执行。`;
          ...
          <div className="approval-detail">
            <div className="approval-detail-head">
              <div>
                ...
                <p className="mono">{detail.approvalId}</p>
              </div>
            ...
            <div className="approval-facts">
              ...
              <div><span>申请人</span><strong>{detail.requesterUserId}</strong></div>
            ...
            <div className="detail-grid approval-identifiers">
              <span>Run ID</span><strong className="mono">{detail.runId}</strong>
              <span>Tool Call ID</span><strong className="mono">{detail.toolCallId}</strong>
              <span>审批人</span><span>{detail.reviewerUserId || '-'}</span>
