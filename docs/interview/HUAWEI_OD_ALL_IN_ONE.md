...
普通套壳通常是用户问题直接发给模型。这个项目在模型前后加了工程化能力：前面有文件解析、知识库检索、记忆上下文；执行中有工具调用，比如查文件、查知识库、查数据库、计算；后面有执行轨迹、质量观测、离线评测和审计。它更像一个可控的数据分析 Agent 后端。
...
> 我更关注的是 AI 后端工程化：模型不能直接碰真实系统，所以我做了工具白名单、SQL 只读限制、RAG 上下文、执行轨迹、质量观测和离线评测。重点是让模型可控、可追踪，而不是自由发挥。
