...
# 推荐启动方式:作者提供的脚本会切换 JDK 17 + 阿里云镜像 + 独立本地仓库
...
- `removeFromStore` 在 InMemory 模式下通过**重建整个 store** 实现删除(LangChain4j 的 InMemoryEmbeddingStore 没有按 ID 删除 API)。Milvus 模式下当前未实现真正删除,只是清 registry,**这里有一个已知不一致**。
...
- **包结构**:所有源码都在 `com.ai.*` 下,`@SpringBootApplication` 在 `com.ai.DataAgentApplication`,扫描整个 `com.ai` 包。新建子包不需要额外 `@ComponentScan`。
...
- **静态前端**:`src/main/resources/static/index.html` 是单文件前端,跟着 jar 一起发布,根路径 `/` 直接访问。改前端无需重启,但 jar 模式下要重新 build。
- **`mvnw17.sh` 是非标准脚本**:它写死了 `JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk-17.0.9.jdk`,在其他机器上需要改路径或用普通 `mvn`。它还指定了 `-s .mvn/settings.xml`(阿里云镜像 + 独立本地仓库),CI 环境下要确认这份 settings 在场。
